...
server <- function(input, output, session) {
  ...
  compute_lod_scores_parallel <- function(data, allele_freqs, num_snps = 50, step_snps = 10, error_rate = 0.01) {
    if (!is.data.table(data)) {
      stop("Error: Input data must be a data.table.")
    }
    if (!is.data.table(allele_freqs)) {
      stop("Error: Allele frequency data must be a data.table.")
    }
    if (nrow(data) == 0) {
    ...
    if (!all(c("rsid", "chromosome", "position", "allele1", "allele2") %in% colnames(data))) {
      stop("Error: Input data is missing required columns.")
    }
    if (!all(c("rsid", "pA", "pG", "pC", "pT") %in% colnames(allele_freqs))) {
      stop("Error: Allele frequency data is missing required columns.")
    }
    ...
    results <- future_lapply(unique(data$chromosome), function(chr) {
      ...
      # Merge allele frequencies and handle missing values
      chr_data <- merge(chr_data, allele_freqs, by = "rsid", all.x = TRUE)
      if (nrow(chr_data) == 0) {
        warning(paste("Warning: No matching SNPs found for chromosome", chr, ". Skipping."))
        return(NULL)
      }
      ...
      for (i in seq_along(start_indices)) {
        ...
        if (nrow(segment_data) < num_snps) {
          next
        }
        
        segment_data[, `:=`(
          p1 = fifelse(allele1 == "A", pA, fifelse(allele1 == "G", pG, fifelse(allele1 == "C", pC, pT))),
          p2 = fifelse(allele2 == "A", pA, fifelse(allele2 == "G", pG, fifelse(allele2 == "C", pC, pT)))
        )]
        
        segment_data[, `:=`(
          P_ROH = fifelse(allele1 == allele2, (1 - error_rate) * p1 + error_rate * p1^2, 2 * error_rate * p1 * p2),
  ...
  estimate_lod_threshold <- function(lod_scores) {
    tryCatch({
      if (!is.data.table(lod_scores)) {
        stop("Error: LOD scores must be a data.table.")
      }
      if (!("lod_score" %in% colnames(lod_scores))) {
        stop("Error: LOD scores data must contain 'lod_score' column.")
      }
      if (nrow(lod_scores) == 0) {
        stop("Error: No LOD scores available for threshold estimation.")
      }
      
      kde_fit <- density(lod_scores$lod_score, kernel = "gaussian")
      if (length(kde_fit$x) == 0 || length(kde_fit$y) == 0) {
        stop("Error: KDE estimation failed due to insufficient data.")
      }
  ...
  # Function to compute LOD scores for user
  compute_lod_scores_for_user <- function(data) {
    tryCatch({
      if (!is.list(data) || !"data" %in% names(data) || !"allele_freqs" %in% names(data)) {
        stop("Error: Input must be a list containing 'data' and 'allele_freqs'.")
      }
      if (!is.data.table(data$data) || !is.data.table(data$allele_freqs)) {
        stop("Error: Both 'data' and 'allele_freqs' must be data.tables.")
      }
      
      lod_data <- compute_lod_scores_parallel(data$data, data$allele_freqs)
      
      if (nrow(lod_data) == 0) {
        stop("Error: No LOD scores computed.")
      }
